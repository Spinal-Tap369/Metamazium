...
def conjugate_gradient(Avp_func, b, max_iter=10, tol=1e-10):
    ...
    b is the first-order grad (already flattened).
    """
...
def build_hessian_vector_product(loss, param_list, damping):
    ...
    Returns (grads, hvp_func):
      ...
      hvp_func: function that takes 'v' and returns H·v + damping·v.
    """
    # Compute first-order gradients with graph retention.
    grads = flat_grad(loss, param_list, create_graph=True, retain_graph=True)

    def hvp_func(v):
        # Clone and detach v to avoid in-place modifications, then require grad.
        v = v.clone().detach().requires_grad_(True)
        # Disable CuDNN for this forward pass to support double-backwards.
...
class TRPO:
    ...
    def __init__(
        ...
        Args:
            ...
            lam: GAE lambda.
            cg_damping: Damping for the Hessian-vector product.
    ...
    def update_policy(self, states, actions, advantages, old_log_probs):
        ...
        Perform one TRPO update:
          ...
          - Use conjugate gradient to obtain a search direction.
          - Use line search to find an acceptable step.
        ...
        def get_loss_kl():
            ...
            loss = -surr.mean()  # We minimize the negative surrogate.
            with torch.no_grad():
                dist_old = Categorical(logits=dist.logits.detach())
            kl = torch.mean(kl_divergence(dist_old, dist))
            return loss, kl

        # Disable CuDNN for the loss and Hessian computation so that double backward is supported.
        with torch.backends.cudnn.flags(enabled=False):
