...
import env  # Ensure environment registration for "MetaMazeDiscrete3D-v0"
...
def main():
    ...
    task_idx = 0  # Index to cycle through tasks sequentially
    ...
    # Buffers for multiple episodes before PPO update
    all_obs = []
    all_actions = []
    all_log_probs = []
    all_values = []
    ...
    while total_steps < total_timesteps:
        # Configure a new maze task for each episode
        ...
        # Estimate next value if episode didn't end naturally
        next_value = 0.0
        if not done:
            obs_for_model = np.transpose(obs_raw, (2, 0, 1))
            obs_torch = torch.from_numpy(obs_for_model).float().unsqueeze(0).to(device)
        ...
        # If enough timesteps collected, perform a PPO update
        if total_steps >= timesteps_per_update:
            ...
            # Stack padded sequences for batch processing
            final_rollouts = {
            ...
            # Perform PPO update and report statistics
            ...
            # Adjust threshold for next update
