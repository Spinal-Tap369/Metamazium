...
from gymnasium.utils import seeding  # Import Gymnasium's seeding utility
...
class MazeBase(object):
    def __init__(self, **kw_args):
        ...
        # Initialize phase-related attributes
        ...
        for _ in it:
            ...
            if self._cell_walls[x, y] > 0:
                pygame.draw.rect(
                    ...
                    pygame.Color("black"),
                    (
                        x * self._render_cell_size,
                        view_size - (y + 1) * self._render_cell_size,
                        self._render_cell_size,
                        self._render_cell_size,
                    ),
                    width=0,
                )
    ...
    def draw_food(self, scr, offset):
        ...
        for _ in it:
            ...
            if self._cur_food_rewards[x, y] > 1.0e-2:
                ...
                pygame.draw.rect(
                    scr,
                    pygame.Color(f, 255, f),
                    (
                        x * self._render_cell_size + offset[0],
                        offset[1] + self._view_size - (y + 1) * self._render_cell_size,
                        self._render_cell_size,
                        self._render_cell_size,
                    ),
                    width=0,
    ...
    def render_trajectory(self, file_name, additional=None):
        ...
        pygame.draw.rect(
            ...
            pygame.Color("red"),
            (
                self._agent_grid[0] * self._render_cell_size,
                self._view_size - (self._agent_grid[1] + 1) * self._render_cell_size,
                self._render_cell_size,
                self._render_cell_size,
            ),
            width=0,
        ...
        if additional is not None:
            for i in range(len(additional["surfaces"])):
                ...
                pygame.image.save(
                    traj_screen, file_name.split(".")[0] + additional["file_names"][i] + ".png"
                )
        else:
